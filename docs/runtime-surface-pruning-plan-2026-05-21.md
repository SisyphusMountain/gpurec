...
| `gpurec/workflow/_stopping_policy.py` | Internal pure optimizer stopping and patience policy shared by the optimizer loop. | Keep direct stopping-policy tests plus optimize helper alias identity before changing patience order or active-batch caps. |
