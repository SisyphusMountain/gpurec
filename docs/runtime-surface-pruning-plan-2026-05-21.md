...
| `gpurec/workflow/config.py` | Public flat `RunConfig` / `SamplingConfig` contract and compatibility facade for config IO, optimizer defaults, and route metadata. | Keep CLI/dataclass surface parity, run-config reference coverage, and template/preflight smokes. |
...
| `gpurec/workflow/_transition_policy.py` | Internal pure workflow transition classification policy behind `_transitions` compatibility imports. | Keep `_transitions` classifier alias identity and transition/checkpoint parity tests before changing classification order. |
| `gpurec/workflow/_hessian_sgd_policy.py` | Internal pure Hessian-SGD workflow threshold policy shared by optimizer loop and transition helpers. | Keep direct policy tests plus Hessian-SGD line-search and warmup workflow parity tests before changing thresholds. |
| `gpurec/workflow/_stopping_policy.py` | Internal pure optimizer stopping and active-batch patience policy shared by the optimizer loop and adaptive rebatch. | Keep direct stopping-policy tests plus optimize/adaptive-rebatch helper alias identity before changing patience order or active-batch caps. |
| `gpurec/workflow/_step_metrics.py` | Internal optimizer-step metric schema helper shared by step execution. | Keep direct step-metric key/type tests plus workflow history parity tests before changing emitted telemetry fields. |
...
| `gpurec/workflow/_schedules.py` | Internal Adagrad restart and loss-stop schedule parser helper behind `gpurec.workflow.config` wrappers. | Keep config facade imports, schedule dataclass pickle identity, and optimizer schedule parity tests before changing parser ownership. |
| `gpurec/workflow/_config_io.py` | Internal JSON/path loading and flat `RunConfig` scalar schema helper behind `gpurec.workflow.config` imports. | Keep config facade imports, JSON/path validation parity, and CLI config diagnostics before changing config IO ownership. |
