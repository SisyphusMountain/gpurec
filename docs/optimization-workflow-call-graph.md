...
flowchart TD
  ...
  ClampBefore["model.clamp_theta_(min_rate, max_rate)<br/>gpurec/api/model.py:2486"]
  EvalBackward["OptimizationRunner._evaluate_and_backward(model)<br/>gpurec/workflow/optimize.py:822"]
  FullLoss["model.full_loss()<br/>gpurec/api/model.py:2318"]
  FullLossFn["_GeneReconFullLossFunction.forward(...)<br/>gpurec/api/model.py:1200"]
  Stream["_stream_full_batches(theta, need_grad=True)<br/>gpurec/api/model.py:1880"]
  ModelEval["_evaluate_static_state(static, theta, need_grad=True)<br/>gpurec/api/model.py:1183"]
  EvalStatic["evaluate_resident_static_state(static, theta, need_grad=True)<br/>gpurec/api/_uniform_evaluator.py:106"]
  ForwardSolve["evaluate_resident_gradient_forward(static, theta)<br/>gpurec/api/_uniform_evaluator.py:78"]
  ...
  FullLossFnBackward["_GeneReconFullLossFunction.backward(...)<br/>gpurec/api/model.py:1210"]
  OptimizerStep["optimizer step<br/>Adam, Adagrad, adagrad-restarts,<br/>hessian-sgd, LBFGS variants"]
  Clear["model.clear()<br/>gpurec/api/model.py:2239"]
