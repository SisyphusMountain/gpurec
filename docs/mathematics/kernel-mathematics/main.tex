...
\end{equation}
It is $-\infty$ when the complementary mass is zero.

The forward does not evaluate $D_x(s)$ as the difference $R_x-A_x(s)$.  For every species
under the lane carrying the row's mass the two terms agree to more digits than the format
has, so the difference is rounding noise and can even change sign.  It uses the second form,
$D_x(s)=\sum_{r\in\mathcal V(s)}q_x(r)$: with the depth-first interval numbering of the
species tree, $\mathcal V(s)$ is the union of the two disjoint sets $\{r:\text{start}(r)>
\text{start}(s)\}$ and $\{r:\text{end}(r)\le\text{start}(s)\}$, and each set's mass is an
exclusive prefix sum of non-negative terms over a fixed species order.  Kernel helper
\kernelref{pi_forward.py}{_compute_total_receiver_mass} supplies the gauge $M_x$;
\kernelref{pi_forward.py}{_write_valid_receiver_prefix_sums} writes the two prefix sums and
\kernelref{pi_forward.py}{_lookup_valid_receiver_mass} reads $D_x(s)$ back out of them.
...
\kernelref{pi_forward.py}{_compute_total_receiver_mass} & Computes the gauge $M_x$ for Equation~\eqref{eq:transfer-complement}. \\
\kernelref{pi_forward.py}{_write_valid_receiver_prefix_sums} & Writes the two prefix sums $D_x(s)$ is assembled from, without subtracting. \\
\kernelref{pi_forward.py}{_lookup_valid_receiver_mass} & Reads $D_x(s)$ per donor and so $\mathcal B[x](s)$ in Equation~\eqref{eq:transfer-complement}. \\
