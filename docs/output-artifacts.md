...
cap for quick programmatic and terminal triage.
...
and effective step cap, `steps_completed`, `elapsed_s`, `best_step`,
`sampling_checkpoint`, and the same final/best NLL and log-likelihood fields,
plus `final_grad_inf`, `final_projected_grad_inf`, and the final validation
source, reason, status, and fallback budget/loss/gradient delta fields, for
