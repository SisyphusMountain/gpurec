...
| `optimization_history.csv` | `optimize`, optimization phase of `run` | CSV version of the in-memory history rows. | Useful for spreadsheets; `history.jsonl` preserves richer typing. Non-finite numeric diagnostics are written as blank CSV cells. |
...
| `run_manifest.json` | `optimize`, optimization phase of `run` | Immutable runtime manifest including command line invocation, config hash, runtime environment, native artifact contract versions, route metadata, final optimization metadata, and reproducibility fields (`torch_seed`, `seeded`, optional `seed_source`, optional `requested_torch_seed`). | Captures the exact `gpurec optimize`/`run` invocation, runtime context, and optimized route contract for audit and rerun verification. |
| `rates_final.tsv` | `optimize`, optimization phase of `run` | Final D/T/L rates, survival probability `pS`, and raw theta values. | Rows are `global`, species labels, or family labels depending on mode. Genewise and specieswise exports fail rather than truncating if theta row counts do not exactly match labels. |
| `per_fam_likelihoods.tsv` | Genewise `optimize` and genewise optimization phase of `run` | Final per-family NLL and log-likelihood in bits. | Genewise-only because rows are independent only in genewise mode. Omitted and stale copies are cleared when the final genewise likelihood evaluation fails with non-finite values. |
...
gradient route, rate parameterization, production default basis, resident batch
route, optimizer, and optimizer-specific settings. Batch route fields include
the default `batch_packing`, `family_chunk_size`, and `clade_budget`.
Optimizer-specific route fields include the
...
optimizer-specific setting overrides reported by
...
metadata, resident batch defaults, and optimizer-specific settings, including
the stored `final_check_iters_e`, must also match the full shipped
likelihood/gradient, resident batch, and optimizer route.
...
the shipped likelihood/gradient contract, resident batch route, and
optimizer-specific route, including the stored `final_check_iters_e`. The
displayed checkpoint route and any route-gate verdicts are derived from the
same audited checkpoint route snapshot. If a legacy checkpoint has no
...
With `--require-production-default-route`, it also rejects changed resident
batching, optimizer-specific settings, stale `final_check_iters_e`, or stale
...
the full shipped likelihood/gradient, resident batch, and optimizer-specific
route before sampling. When both route gates are requested, standalone sampling
audits the checkpoint route once and reuses that verdict for both checks.
