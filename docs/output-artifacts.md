...
| `run_manifest.json` | `optimize`, optimization phase of `run` | Immutable runtime manifest including command line invocation, config hash, runtime environment, route metadata, final optimization metadata, and reproducibility fields. | Captures the exact `gpurec optimize`/`run` invocation and the optimized route contract for audit and rerun verification. |
