...
| Composite scheduled fallback-budget route | Same prefix and tail, `lbfgsb_loss_change_tol_schedule=0.25:2,0.1:2`, `lbfgsb_high_kkt_stop_patience=2`, `lbfgsb_fallback_max_coordinates=4`, `lbfgsb_fallback_max_loss_evals=4` | `1491.84s` process wall | `1699456.25` fixed32 validated | `1.5397067070007324` | New best clean one-command endpoint. The first loss schedule phase advanced at step 69 after two coarse `0.25`-bit plateau rows; the tighter phase reached best step 90, then stopped by high-KKT tiny-progress patience after two budget-exhausted no-movement fallback rows. |
| Composite scheduled fallback-budget default factor rerun | Same scheduled route after making `lbfgsb_fallback_resolution_competition_factor=0` the default | `1304.67s` process wall | `1699458.625` fixed32 validated | `2.8339502811431885` | Faster clean current-HEAD route, but `2.375` bits above the best clean scheduled endpoint. It used one accepted fallback row at step 68, reached best step 82, and stopped by high-KKT tiny-progress patience. Same-control, factor-16, and no-high-KKT resumes from the best checkpoint did not improve beyond `1699458.625`, so this is a real fast plateau rather than just an early-stop artifact. |
...
| Composite one-repair no-force route without coordinate fallback | Same no-force one-repair route, but `lbfgsb_fallback_max_coordinates=0` | `1469.20s` process wall (`1467.0250371850561s` optimizer elapsed) | `1699446.375` fixed32 validated | `0.9117991924285889` | Near-best Pareto route. It also accepted no fallback rows, reached best step 90, and stopped by ordinary loss-change patience. Disabling coordinate fallback changed the trajectory slightly and landed `0.375` bits above the lowest endpoint, but it saved `21.61s`; the terminal no-move rows cost about `16.2s` each instead of about `24.5s`. |
| Composite one-repair no-force route without coordinate fallback, max line search 3 | Same no-coordinate one-repair route, but `lbfgs_max_ls=3` | `1503.43s` process wall (`1501.1421133280965s` optimizer elapsed) | `1699445.875` fixed32 validated | `0.7720391750335693` | New lowest clean one-command endpoint, but slower than the near-best Pareto route. The tighter L-BFGS-B line-search cap changed the basin instead of merely trimming terminal overhead: it reached best step 91 at `1699445.875`, then spent two no-move line-search-failed rows of about `22.4s` each before final validation. |
| Composite one-repair no-force route with one-row final patience | Same no-force one-repair route, but `lbfgsb_loss_change_tol_schedule=0.25:1,0.1:1` | `1437.82s` process wall (`1435.6508579850197s` optimizer elapsed) | `1699446.875` fixed32 validated | `1.9759225845336914` | Faster Pareto point, not a new lowest endpoint. It reached best step 88, accepted one projected-gradient fallback row in the final phase, then stopped by high-KKT tiny-progress patience. It is `52.99s` faster than the two-row final-patience no-force route, but `0.875` bits higher. |
| One-row final-patience route, preserved-state no-high-KKT resume | Resume the one-row final-patience best checkpoint with `lbfgsb_high_kkt_stop_patience=0` and the two-row final patience schedule | `49.59s` process wall for the resume | `1699446.875` fixed32 validated | `1.9759092330932617` | Rejected as a segmented polish route. Restoring the best checkpoint and disabling high-KKT stop only replayed two no-improvement rows and stopped by ordinary loss-change patience, so the `1699446.875` endpoint is a real local plateau for that trajectory. |
...
| Composite scheduled fallback-budget, forced factor-16 fallback with one-row high-KKT terminal stop | Same forced factor-16 route with `lbfgsb_high_kkt_stop_patience=1` | `1345.31s` process wall | `1699457.75` fixed32 validated | `1.3558012247085571` | New middle Pareto point. It is slower than the current fast forced route by `49.68s`, but `0.5` bits lower. The route advanced the loss schedule at step 73, accepted one forced fallback at step 74, then ordinary L-BFGS-B reached best step 84 before the first no-move high-KKT fallback stopped the run. Because the trajectory diverged before the terminal stop, this is a time/objective tradeoff rather than a pure one-row trimming of the faster route. |
| Composite scheduled fallback-budget, forced factor-16 fallback with best retry | Same forced factor-16 route plus `lbfgsb_best_retry_attempts=1` | `1250.68s` process wall | `1699464.5` fixed32 validated | `2.833920955657959` | Rejected as a low-likelihood endpoint. The retry fired once, but the run's best checkpoint was already a weaker `1699464.5`-bit point; replaying it did not escape, so best-checkpoint retry remains off for the current fast route. |
| Composite scheduled fallback-budget, resolution factor 8 | Same scheduled route with `lbfgsb_fallback_resolution_competition_factor=8` | `1364.59s` process wall | `1699458.875` fixed32 validated | `2.344407558441162` | Rejected as dominated by the factor-0 fast rerun: it was about `59.92s` slower and `0.25` bits higher. It accepted five fallback rows, reached best step 81, and stopped by high-KKT tiny-progress patience, so the intermediate factor paid fallback cost without reaching the lower factor-16 basin. |
| Composite scheduled fallback-budget, resolution factor 4 | Same scheduled route with `lbfgsb_fallback_resolution_competition_factor=4` | `976.55s` process wall | `1699470.5` fixed32 validated | `6.247040748596191` | Rejected as an early plateau. It accepted no fallback rows, advanced the schedule once, then spent two no-move tight-phase fallback rows and stopped by ordinary loss-change patience. This confirms that a small resolution factor does not trigger the fallback chain needed for the lower basin. |
| Composite scheduled fallback-budget rerun after final-check baseline skip | Same scheduled route under the final-check duplicate-evaluation skip | `1156.33s` process wall | `1699461.5` fixed32 validated | `2.5096702575683594` | Rejected as a low-likelihood endpoint. The run is a true uniform-`0.05` trajectory and the final fixed32 check correctly recorded `final_check_status=baseline` with `0` extra final-check evaluations, but the L-BFGS-B tail stopped at best step 72 after two budget-exhausted no-movement fallback rows rather than continuing to the earlier step-90 basin. |
| Composite scheduled fallback-budget rerun larger-fallback rescue | Resume the rejected rerun's best checkpoint with `lbfgsb_fallback_max_coordinates=16` and `lbfgsb_fallback_max_loss_evals=8` | `81.25s` process wall | `1699461.5` fixed32 validated | `2.5096702575683594` | Rejected diagnostic. The larger fallback budget tried two more tail rows from the rerun's best checkpoint and still found no improving move, so that trajectory is treated as genuinely stuck rather than merely under-budgeted. |
| Composite scheduled fallback-budget with resolution-aware fallback competition | Same scheduled route with `lbfgsb_fallback_resolution_competition_factor=16` and `lbfgsb_fallback_max_loss_evals=4` | `1707.41s` process wall | `1699456.0` fixed32 validated | `0.45844364166259766` | New lowest clean one-command endpoint, but slower than the previous scheduled route. The run used `12` accepted fallback rows after the schedule transition, reached best step 98, and stopped by high-KKT tiny-progress patience. This is a quality win over `1699456.25`, not a time win, so the factor is now an explicit quality-polish knob rather than the fast-route default. |
| Composite resolution-aware fallback competition, budget 3 | Same route with `lbfgsb_fallback_resolution_competition_factor=16` and `lbfgsb_fallback_max_loss_evals=3` | `1162.68s` process wall | `1699465.0` fixed32 validated | `6.007434368133545` | Rejected as a low-likelihood endpoint. Budget 3 was enough to improve beyond budget 2 but not enough to reach the lower basin; it stopped by high-KKT tiny-progress patience. |
| Composite resolution-aware fallback competition, budget 2 | Same route with `lbfgsb_fallback_resolution_competition_factor=16` and `lbfgsb_fallback_max_loss_evals=2` | `1048.87s` process wall | `1699467.625` fixed32 validated | `2.34967041015625` | Rejected as dominated. It was near the objective-plateau route's wall-time band but had a worse final objective and accepted no fallback rows before ordinary loss-change patience stopped the run. |
| Composite scheduled fallback-budget preserved-state polish | Resume the scheduled route's best checkpoint with the same tail controls, restoring serialized L-BFGS-B state | `1562.68s` combined (`1491.84s + 70.84s`) | `1699455.875` fixed32 validated | `1.8008732795715332` | Lowest measured endpoint so far, but segmented from the prior best checkpoint. The duplicate final fixed32 validation is now skipped when the baseline row already used the requested final-check budget, reducing the measured resume polish from `86.62s` to `70.84s`. Disabling the high-KKT stop for the same resume stopped by ordinary loss patience at the same `1699455.875` in `102.26s`, so the extra row did not improve beyond this point. |
| Composite scheduled fallback-budget with best retry | Same scheduled route plus `lbfgsb_best_retry_attempts=1` | `1249.01s` process wall | `1699465.5` fixed32 validated | `2.835155487060547` | Rejected as a low-likelihood endpoint. The retry fired and restored the best checkpoint, but this run advanced into a weaker tail trajectory and the replay did not escape it. Useful evidence that best-checkpoint retry is not enough without a good schedule-transition fallback. |
| Composite scheduled fallback-budget, force fallback after schedule advance | Same scheduled route plus `lbfgsb_loss_schedule_force_fallback=true` and `lbfgsb_best_retry_attempts=1` | `1436.61s` process wall | `1699457.75` fixed32 validated | `3.1008167266845703` | Faster than the best clean scheduled endpoint but `1.5` bits higher. The schedule advanced at step 70 and forced a projected-gradient fallback at step 71, which recovered the lower basin, but the terminal best-checkpoint retry at step 84 made no additional progress. Treat as a time/objective tradeoff, not the current best optimum route. |
| Composite scheduled fallback-budget, force fallback without retry | Same route with `lbfgsb_loss_schedule_force_fallback=true` and no best-checkpoint retry | `1016.11s` process wall | `1699468.125` fixed32 validated | `8.011322021484375` | Rejected as dominated. Removing retry saved time, but the forced fallback never accepted a move after schedule advance and ordinary loss-change patience stopped the run in the weaker basin. |
| Composite scheduled fallback-budget, delayed coarse plateau | Same scheduled route plus `lbfgsb_loss_change_tol_schedule=0.25:3,0.1:2` and `lbfgsb_best_retry_attempts=1` | `1216.21s` process wall | `1699463.5` fixed32 validated | `0.6850919723510742` | Rejected as a low-likelihood endpoint. The delayed coarse schedule reached a lower handoff by step 70, but then advanced at step 73 and immediately plateaued in the tight phase; the best-checkpoint retry restored step 74 and did not move. |
| Composite scheduled fallback-budget route, impatient polish | Same as above, but `lbfgsb_loss_change_tol_schedule=0.25:2,0.1:1` | `1185.51s` process wall | `1699463.25` fixed32 validated | `2.8354413509368896` | Faster intermediate tradeoff, but rejected as the best endpoint because the one-row tight plateau phase switched into high-KKT stop much earlier. It is lower than the plain fallback-budget route but much higher than the `0.1:2` scheduled endpoint. |
| Composite fallback-budget segmented polish | Objective-plateau route, then resume with `loss_change_tol=0.1`, `lbfgsb_fallback_max_coordinates=4`, `lbfgsb_fallback_max_loss_evals=4`, and continue to high-KKT stop | `1522.42s` combined (`1039.92s + 409.25s + 73.25s`) | `1699456.5` fixed32 validated | `2.7642757892608643` | Superseded by the scheduled route. The first continuation reached `1699456.875` by step 89 and hit a max-step cap; the second continuation stopped cleanly by `lbfgsb_high_kkt_tiny_progress_patience` at fixed32 `1699456.5`. |
...
| Composite limited-coordinate continuation | Resume the objective-plateau route, `loss_change_tol=0.1`, `lbfgsb_high_kkt_stop_patience=2`, `lbfgsb_fallback_max_coordinates=4` | Probe killed after step 79; rows 69-79 cost `207.30s` before the next row stalled | `1699459.875` at last checkpoint | `1.2936291694641113` | Rejected as not clean enough. It improved below the no-coordinate full route, but coordinate limit 4 still entered a disproportionate row after step 79. The probe also verified that old L-BFGS-B checkpoints need backward-compatible defaults for the new `fallback_max_coordinates` optimizer key. |
...
For a practical dataset-independent tail stop, the one-shot routes with
`loss_patience=2` and relaxed `projected_grad_tol=10` all stopped automatically
in the same time band.  The `loss_change_tol=1` route reached `1699470.125`
...
objective-plateau route.  Adding `lbfgsb_fallback_max_loss_evals=4` gives a
clean fallback-budget tradeoff: `1699464.875` bits in `1125.67s` from uniform
`0.05`.  It is slower than the objective-plateau route, but it buys `1.5` more
bits and avoids unbounded fallback competition by spending at most four
fallback loss-only probes per optimizer row.
...
segmented endpoint: `1699456.5` bits in `1522.42s` combined.
Scheduling the L-BFGS-B loss stop makes that polish automatic.  With
`lbfgsb_loss_change_tol_schedule=0.25:2,0.1:2`, the workflow advances from a
coarse plateau to a tighter plateau in the same optimizer state and reaches
`1699456.25` bits in `1491.84s` from uniform `0.05`.  The final two optimizer
rows are no-movement fallback-budget rows, so the route stops by the high-KKT
tiny-progress guard rather than by a hard step cap.  Tightening the second
schedule phase to `0.1:1` saves time (`1185.51s`) but stopped much higher at
`1699463.25` bits, so the accepted low-objective route remains `0.25:2,0.1:2`.
...
- The best `test_trees_1000` outer route now switches from the short E7/Pi4
  Adagrad warmup through a fixed8 bridge and exactly one E16/Pi8 repair row into
  L-BFGS-B with a larger step scale (`lr=0.6`).  That differs from HOGENOM's
  accepted pattern, where the useful basin came from a longer tied-budget ladder
  and high-fidelity polish rather than an early aggressive L-BFGS-B tail.
- For specieswise use across datasets, `test_trees_1000` now has a working
  staged likelihood-stall route: the early L-BFGS-B tail can use
  `lbfgsb_loss_change_tol_schedule=0.25:1,0.1:2` with a relaxed
  projected-gradient guard instead of a hand-counted number of tail steps.  The
  fidelity prefix is still explicit, but the successful promotion is intentionally
  short: one E16/Pi8 row was better than continuing the repair phase.  That is
  again different from HOGENOM's route, where fixed high-fidelity phase lengths
  were the accepted tuning handle.
...
- Late L-BFGS-B fallback behavior is currently dataset-specific.  On
  ...
  length.  A best-checkpoint retry and a schedule-transition forced fallback
  were tested as additional tail escape hatches; they improved some trajectories
  but did not replace the scheduled fallback-budget route as the lowest clean
  one-command endpoint.  A later rerun with the duplicate final-check skip
  confirmed that the route can still land in a weaker tail (`1699461.5` bits)
  when the fallback handoff stalls early; increasing the rescue fallback budget
  from that checkpoint did not recover the lower basin.  With the default
  resolution-competition factor restored to `0`, a current-HEAD rerun gives a
  faster `1304.67s` endpoint but stops at `1699458.625` bits; short resumes
  from that checkpoint did not improve it.  Pairing the schedule-transition
  ...
  competition, reached `1699446.0` bits in `1490.81s`.  Reducing `lbfgs_max_ls`
  to `3` on the same no-coordinate policy found the current lowest endpoint,
  `1699445.875` bits, but raised wall time to `1503.43s`; the line-search cap
  changed the basin rather than just trimming overhead.  These routes accepted
  no fallback rows; the important change was the one-row E16/Pi8 handoff, which
  avoided the second repair row and let the ordinary L-BFGS-B tail continue to
  best step 90 or 91.
  Disabling coordinate fallback on that route is the current near-best Pareto
  point (`1469.20s`, `1699446.375` bits): it still accepted no fallback rows,
  reached best step 90, and made the terminal no-move rows cheaper, but the
  slightly different trajectory landed `0.375` bits above the lowest endpoint.
  Tightening the final phase to one-row patience is a faster Pareto point
  (`1437.82s`, `1699446.875` bits), but it entered a nearby fallback-assisted
  plateau and a no-high-KKT resume did not recover the lower `1699446.0` basin.
  ...
  by the explicit `lbfgsb_fallback_resolution_competition_factor` knob and kept
  off for the faster route.  Intermediate factors did not give a useful middle
  ground: factor `4` accepted no fallback rows and stopped at `1699470.5` bits,
  while factor `8` accepted five fallback rows but still ended slower and
  higher than the factor-`0` fast route.  Reducing that fallback
  loss-evaluation budget to `2` or `3` saved time but stopped at higher
  objectives.  Adding one best-checkpoint retry to the forced factor-`16` route
  also failed to help: it fired from a weaker `1699464.5`-bit checkpoint and
  did not escape.  Disabling coordinate fallback avoids tail risk but stopped
  at a higher objective in the full run.  These high-KKT/fallback guards are
