...
| Composite scheduled fallback-budget preserved-state polish | Resume the scheduled route's best checkpoint with the same tail controls, restoring serialized L-BFGS-B state | `1578.46s` combined (`1491.84s + 86.62s`) | `1699455.875` fixed32 validated | `1.8010296821594238` | Lowest measured endpoint so far, but segmented from the prior best checkpoint. Disabling the high-KKT stop for the same resume stopped by ordinary loss patience at the same `1699455.875` in `102.26s`, so the extra row did not improve beyond this point. |
...
- Late L-BFGS-B fallback behavior is currently dataset-specific.  On
  ...
  one-command endpoint.  Disabling coordinate fallback avoids tail risk but
  stopped at a
