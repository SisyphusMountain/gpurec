...
| Composite scheduled fallback-budget with resolution-aware fallback competition | Same scheduled route after making accepted L-BFGS-B fallback moves compete when their decrease is only fp32-resolution-scale; `lbfgsb_fallback_max_loss_evals=4` | `1707.41s` process wall | `1699456.0` fixed32 validated | `0.45844364166259766` | New lowest clean one-command endpoint, but slower than the previous scheduled route. The run used `12` accepted fallback rows after the schedule transition, reached best step 98, and stopped by high-KKT tiny-progress patience. This is a quality win over `1699456.25`, not a time win. |
| Composite resolution-aware fallback competition, budget 3 | Same route with `lbfgsb_fallback_max_loss_evals=3` | `1162.68s` process wall | `1699465.0` fixed32 validated | `6.007434368133545` | Rejected as a low-likelihood endpoint. Budget 3 was enough to improve beyond budget 2 but not enough to reach the lower basin; it stopped by high-KKT tiny-progress patience. |
| Composite resolution-aware fallback competition, budget 2 | Same route with `lbfgsb_fallback_max_loss_evals=2` | `1048.87s` process wall | `1699467.625` fixed32 validated | `2.34967041015625` | Rejected as dominated. It was near the objective-plateau route's wall-time band but had a worse final objective and accepted no fallback rows before ordinary loss-change patience stopped the run. |
...
- Late L-BFGS-B fallback behavior is currently dataset-specific.  On
  ...
  many fallback rows and raised wall time to `1707.41s`.  Reducing that fallback
