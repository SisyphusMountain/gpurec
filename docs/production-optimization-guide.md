...
the final phase to one-row patience gives a faster Pareto point (`1437.82s`,
`1699446.875` bits), but a preserved-state no-high-KKT resume did not recover
the lower endpoint from that plateau. Plain factor `16` remains a slower
quality-polish setting. Intermediate probes at factors `4` and `8` did not
preserve the lower objective basin, so this is not currently a smooth
