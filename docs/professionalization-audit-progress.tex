...
CPU-only audit gate &
\texttt{33 passed after NLL alias cleanup} \\
\texttt{pytest --collect-only -q} &
Collection after recursion exclusions &
\texttt{60 tests collected in 0.77s} \\
...
\begin{enumerate}[leftmargin=*]
  \item Add a low-risk \texttt{UniformChunkedReconModel.from\_trees} alias and
        constructor smoke coverage.
  \item Move batch-planning logic toward one shared implementation used by both
        high-level model APIs.
  \item Add public model accessors or context managers for backtracking and
        workflow code currently using private attributes.
  \item Rename or alias \texttt{compute\_log\_likelihood} to reflect its NLL
        contract without breaking existing callers.
