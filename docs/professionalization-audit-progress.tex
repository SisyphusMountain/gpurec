...
CPU-only audit gate &
\texttt{34 passed after model-access cleanup} \\
\texttt{pytest --collect-only -q} &
Collection after recursion exclusions &
\texttt{61 tests collected in 0.79s} \\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Add a model-owned \texttt{reconciliation\_state()} result object so
        backtracking can also stop importing private parameter extraction
        helpers.
  \item Curate the remaining HOGENOM/W\&B helper scripts and configs into
        source-like experiment tooling, or explicitly ignore/drop generated
        artifacts.
