...
The latest audit slice found three high-value follow-ups that remain open:
make the promoted HOGENOM workflows discoverable from \texttt{README.md} and
packaging metadata, auto-apply the registered \texttt{unit} and
\texttt{integration} pytest markers by test path, and either restore or rename
the currently hollow specieswise-uniform GPU parity test.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Add a README HOGENOM workflow matrix explaining the installed CLI,
        ...
        harness.
  \item Auto-mark unit and integration tests by directory so
        \texttt{pytest -m "unit and not gpu"} becomes a useful CPU gate.
