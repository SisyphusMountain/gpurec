...
\texttt{python -m py\_compile ...HOGENOM W\&B scripts...} &
Curated experiment launcher syntax &
\texttt{ok} \\
\texttt{pytest -q tests/unit/test\_workflow.py} &
Workflow and public-surface source guards &
\texttt{14 passed after W\&B launcher cleanup} \\
\texttt{CUDA\_VISIBLE\_DEVICES='' pytest -q ...CPU fast gate...} &
CPU-only audit gate &
\texttt{37 passed after W\&B launcher cleanup} \\
...
\begin{enumerate}[leftmargin=*]
  \item Finish curating the remaining HOGENOM helper scripts, analysis docs,
        and modified notebooks into tracked source or explicitly ignored local
        artifacts.
