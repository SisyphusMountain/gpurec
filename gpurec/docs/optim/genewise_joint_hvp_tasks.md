...
- [ ] **Step 1: Read FIRST** the α path in `hvp()` — `use_receiver_weights`, `u_alpha`, `dreceiver_log_probs`, `out_receiver_weights`, and `d_grad_receiver_log_probs` accumulation. The α HVP is validated only with **specieswise** θ (`_verify_hvp_recv`); under genewise θ the per-family cotangent reductions (`d_cot_receiver_log_probs`, `grad_col`) have never been exercised. Expect a P0-style per-family reduction bug and locate it before patching.
