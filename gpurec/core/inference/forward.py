...
def pi_wave_forward(
    ...
    *,
    ...
    # The four constants the two tree walks read together, side by side per species.
    ...
    # The exact solver records rows which need its automatic log-space range fallback.
    ...
    while True:
        ...
        for meta in wave_layout["wave_metas"]:
            ...
            # Log-space prologue: the leaf initializer (iteration 0) for a leaf
            # wave, plus the gene-split-input step (iteration 1) for a split wave, which is also what
            # publishes ``dts_center_offset``.
            # The exact solve needs only the leaf initializer or the split-input step that
            # publishes ``dts_center_offset``. ``pi_iters`` only bounds the rare fallback.
            ...
            # The prologue's last write lands in ``pibar`` for a leaf wave and in ``pi`` for a
            # split wave.
            ...
            # On the redo pass, log-space sweeps fill only rows the exact kernel rejected.
            fallback_end = pi_iters if pi_iters > prologue_iters else prologue_iters + 2
            if sweep_every_wave:
