...
@triton.jit
def _wave_backward_uniform_tree_make_u_kernel(
    term_in_ptr,
    active_mask_ptr,
    pibar_coeff_ptr,
    pibar_corr_ptr,
    ...
    s_block = tl.program_id(1)
    rows = row_block * TILE_W + tl.arange(0, TILE_W)
    s_offs = s_block * BLOCK_S + tl.arange(0, BLOCK_S)
    row_valid = rows < W
    species_valid = s_offs < S
    if USE_ACTIVE_MASK:
    ...
    term_val = tl.load(term_in_ptr + offsets, mask=mask, other=0.0).to(DTYPE)
    coeff = tl.load(pibar_coeff_ptr + offsets, mask=mask, other=0.0).to(DTYPE)
    u_d = term_val * coeff
    tl.store(pibar_corr_ptr + offsets, tl.where(mask, u_d, tl.zeros_like(u_d)), mask=store_mask)
    tile_A = tl.sum(tl.where(mask, u_d, tl.zeros([BLOCK_S, TILE_W], dtype=DTYPE)), axis=0)
    tl.atomic_add(A_ptr + rows, tile_A, sem="relaxed", mask=row_mask)


@triton.jit
def _wave_backward_uniform_tree_reduce_level_kernel(
    pibar_corr_ptr,
    active_mask_ptr,
    compact_level_ptr,
    compact_level_parent_ptr,
    ...
    level_start = tl.load(compact_level_ptr + level)
    level_end = tl.load(compact_level_ptr + level + 1)
    node_offs = level_start + node_block * BLOCK_NODES + tl.arange(0, BLOCK_NODES)
    node_mask = node_offs < level_end
    parent = tl.load(compact_level_parent_ptr + node_offs, mask=node_mask, other=0)
    c1 = tl.load(compact_level_child1_ptr + node_offs, mask=node_mask, other=S)
    c2 = tl.load(compact_level_child2_ptr + node_offs, mask=node_mask, other=S)
...
@triton.jit
def _wave_backward_uniform_tree_combine_base_kernel(
    term_in_ptr,
    term_out_ptr,
    active_mask_ptr,
    diag_ptr,
    ...
    p_prime = tl.load(p_prime_ptr + offsets, mask=mask, other=0.0).to(DTYPE)
    corr = tl.load(pibar_corr_ptr + offsets, mask=mask, other=0.0).to(DTYPE)
    A = tl.load(A_ptr + rows, mask=row_mask, other=0.0).to(DTYPE)
    base = term_val * diag_wt + p_prime * (A[None, :] - corr)
    tl.store(term_out_ptr + offsets, tl.where(mask, base, tl.zeros_like(base)), mask=store_mask)


@triton.jit
def _wave_backward_uniform_tree_spec_scatter_kernel(
    term_in_ptr,
    term_out_ptr,
    active_mask_ptr,
    sl1_ptr,
...
@triton.jit
def _wave_backward_uniform_tree_accumulate_kernel(
    term_out_ptr,
    active_mask_ptr,
    v_k_ptr,
    W: tl.constexpr,
...
def _wave_backward_uniform_tree_staged_prototype(
    Pi_star, Pibar_star, ws, W, S,
    dts_r,
    rhs,
    mt_squeezed, DL_const, Ebar, E, SL1_const, SL2_const,
    ...
    *,
    ...
    """Opt-in Proposal 1 prototype: staged tree-DP self-loop J^T."""
    mode = "staged"
    if not _self_loop_prototype_requested(mode):
        return None
    if Pi_star.device.type != "cuda":
        return _self_loop_prototype_unavailable(mode, "GPUREC_SELF_LOOP_TREE_STAGED requires CUDA tensors")
    if Pi_star.dtype not in (torch.float32, torch.float64):
        return _self_loop_prototype_unavailable(
            mode,
            "GPUREC_SELF_LOOP_TREE_STAGED currently supports fp32/fp64 only",
        )
    if accum_param_grads is not None:
        return _self_loop_prototype_unavailable(
            mode,
            "GPUREC_SELF_LOOP_TREE_STAGED returns per-element parameter VJPs; "
            "disable in-kernel self-loop parameter accumulation for this prototype",
        )
    if const_layout not in (0, 1, 2):
        return _self_loop_prototype_unavailable(mode, "unsupported self-loop constant layout")
    ...
    compact_level_child1 = compact_level_child1.to(device=device, dtype=torch.int32).contiguous()
    compact_level_child2 = compact_level_child2.to(device=device, dtype=torch.int32).contiguous()
    level_ptr_cpu = compact_level_ptr.detach().cpu().tolist()
    n_levels = len(level_ptr_cpu) - 1

    tile_w = triton.next_power_of_2(
        max(1, int(os.environ.get("GPUREC_SELF_LOOP_TREE_TILE_W", "2")))
    )
    tile_s = max(1, int(os.environ.get("GPUREC_SELF_LOOP_TREE_TILE_S", "128")))
    block_s = min(triton.next_power_of_2(tile_s), triton.next_power_of_2(S))
    block_nodes = triton.next_power_of_2(
        max(1, int(os.environ.get("GPUREC_SELF_LOOP_TREE_BLOCK_NODES", "128")))
    )
    n_row_blocks = triton.cdiv(W, tile_w)
    n_s_blocks = triton.cdiv(S, block_s)
    scratch_shape = (W, S)
    ...
    if pibar_corr is None:
        pibar_corr = torch.empty(scratch_shape, device=device, dtype=dtype)
    pibar_A = _scratch_view(scratch, "self_loop_tree_A", (W,), device=device, dtype=dtype)
    if pibar_A is None:
        pibar_A = torch.empty((W,), device=device, dtype=dtype)
    ...
    precompute_block_s = triton.next_power_of_2(S)
    precompute_warps = int(os.environ.get("GPUREC_SELF_LOOP_TREE_PRECOMPUTE_WARPS", "8"))
    precompute_options = {}
    if precompute_warps > 0:
        precompute_options["num_warps"] = precompute_warps
    ...
    staged_warps = int(os.environ.get("GPUREC_SELF_LOOP_TREE_NUM_WARPS", "4"))
    staged_options = {}
    if staged_warps > 0:
        staged_options["num_warps"] = staged_warps
    reduce_warps = int(os.environ.get("GPUREC_SELF_LOOP_TREE_REDUCE_WARPS", "4"))
    reduce_options = {}
    if reduce_warps > 0:
        reduce_options["num_warps"] = reduce_warps

    for n in range(int(neumann_terms)):
        term_in = rhs if n == 0 else (spec_buf if n % 2 == 1 else term_buf)
        term_out = spec_buf if n % 2 == 0 else term_buf
        pibar_A.zero_()
        _wave_backward_uniform_tree_make_u_kernel[(n_row_blocks, n_s_blocks)](
            term_in,
            active_mask if active_mask is not None else rhs,
            aw1,
            pibar_corr,
        ...
        for level in range(n_levels):
            level_width = int(level_ptr_cpu[level + 1] - level_ptr_cpu[level])
            if level_width <= 0:
                continue
            _wave_backward_uniform_tree_reduce_level_kernel[
                (n_row_blocks, triton.cdiv(level_width, block_nodes))
            ](
                pibar_corr,
                active_mask if active_mask is not None else rhs,
                compact_level_ptr,
        ...
        _wave_backward_uniform_tree_combine_base_kernel[(n_row_blocks, n_s_blocks)](
            term_in,
            term_out,
            active_mask if active_mask is not None else rhs,
            aw0,
        ...
        _wave_backward_uniform_tree_spec_scatter_kernel[(n_row_blocks, n_s_blocks)](
            term_in,
            term_out,
            active_mask if active_mask is not None else rhs,
            aw3,
        ...
        _wave_backward_uniform_tree_accumulate_kernel[(n_row_blocks, n_s_blocks)](
            term_out,
            active_mask if active_mask is not None else rhs,
            v_k,
            W,
...
def wave_backward_uniform_fused(
    ...
    prototype_result = _wave_backward_uniform_tree_staged_prototype(
        Pi_star,
        Pibar_star,
        ws,
        W,
