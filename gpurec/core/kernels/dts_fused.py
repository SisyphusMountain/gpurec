...
def _dts_fused_kernel(
    ...
    """Compute per-split DTS rows for the small-wave fast path."""
    n = tl.program_id(0)
    ...
    t1 = pi_l + pibar_r
    t2 = pi_r + pibar_l
    t3 = log_pS_s + pi_l_c1 + pi_r_c2
    t4 = log_pS_s + pi_r_c1 + pi_l_c2
...
def dts_fused(
    ...
    """Materialized per-split DTS fast path for small waves.

    The lean parameter layouts are shared species constants or family-indexed
    scalar/species constants. Parent reduction happens in the caller.
    """
    N = lefts.shape[0]
    S = Pi.shape[1]
    if out is None:
