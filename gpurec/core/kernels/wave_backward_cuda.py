"""Runtime CUDA kernels for experimental wave-backward paths.

These kernels are intentionally opt-in.  They are compiled with NVRTC through
the CUDA Python bindings so the package does not need an nvcc-backed extension
build step.
"""
    ...
    if int(err) != 0:
        ...
        if prog is not None:
            try:
                ...
                size = _check_nvrtc(nvrtc.nvrtcGetProgramLogSize(prog), "nvrtcGetProgramLogSize")[0]
                buf = bytearray(size)
...
def _load_cuda_kernel(device_index: int, cc_major: int, cc_minor: int):
    ...
    del device_index  # Part of the cache key; modules are context-specific.
...
def wave_backward_uniform_nosplit_cuda(
    ...
    *,
    ...
    """Run the experimental no-split uniform self-loop CUDA kernel.

    This prototype supports the production global/uniform fp32 path with
    in-kernel parameter accumulation.  It intentionally does not materialize
    the six per-element contribution tensors.
    """
    if accum_param_grads is None:
        raise ValueError("CUDA no-split wave prototype requires accum_param_grads")
    float_tensors = (
        Pi_star,
        Pibar_star,
        rhs,
    ...
    if any(t.dtype != torch.float32 for t in float_tensors):
        raise ValueError("CUDA no-split wave prototype currently supports fp32 only")
    if Pi_star.device.type != "cuda":
        raise ValueError("CUDA no-split wave prototype requires CUDA tensors")
    if active_mask is not None and active_mask.dtype is not torch.bool:
        raise ValueError("active_mask must be a bool tensor")
    if accum_param_grads[0].numel() != 1 or accum_param_grads[1].numel() != 1:
        raise ValueError("CUDA no-split wave prototype requires scalar log_pD/log_pS grads")
    ...
    with torch.cuda.device(device_index):
        ...
        # Required for S=1999: the prototype uses about 56 KiB dynamic shared memory.
        _check_driver(
