...
def vjp_root_to_theta(static, sv, seed_root, theta, receiver_weights, *, drop_norm=True,
    ...
    return implicit_grad_loglik_vjp_wave(
        ...
        # Always True on this path, and not a knob: every consumer of this VJP contracts the
        # receiver cotangent. build_point_cache stores it as cache["accum"]["grad_col"] and
...
def make_ggn_hvp(static, theta, receiver_weights, sv, *, self_tol=None,
    ...
    def hvp(v_vec):
        ...
        t = jvp_root_scores(static, theta, v, sv, self_tol=self_tol, self_max_iter=self_max_iter,
                            # The Gauss-Newton operator re-solves the tangent at a fresh theta on
                            # every call, so there are no primal gene-split rows worth keeping.
                            primal_gene_split=None,
