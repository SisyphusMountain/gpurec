...
def fit(batch_statics, theta0, receiver_weights, *, pi_iters=128, neumann_terms=64,
        adam_steps=300, lbfgs_iters=300, deflate_rounds=0, lam_margin=1.3, lam_floor=1e-3,
        lanczos_m=200, certify_m=200, init_rate=None, seed=0, verbose=True, out=None, label="model"):
    ...
    static = batch_statics
    dev = theta0.device
    col_weights = receiver_weights.float().contiguous()
    S = int((static[0] if isinstance(static, (list, tuple)) else static).species_helpers["S"])
    p = 3 * S
    so = (static[0] if isinstance(static, (list, tuple)) else static).solver_options
    ...
    if verbose:
        ...
        print(f"=== specieswise fit: {label}  S={S} p={p}  pi={pi_iters} neumann={neumann_terms} "
    ...
    report = {
        "size": label, "S": S, "p": p, "pi_iters": pi_iters, "neumann_terms": neumann_terms,
