...
| `hessian-sgd` | Projected Hessian-conditioned gradient steps for genewise batches. | Requires `mode=genewise`; finite-difference 3x3 row Hessians refresh every `fd_hessian_refresh_steps`, receive BFGS row updates between refreshes, and precondition fresh gradients with step scale `lr`. Normal full-stage steps can use `hessian_sgd_normal_fixed_iters_pi` and `hessian_sgd_normal_neumann_terms`; polish uses `hessian_sgd_polish_refresh_steps`, `hessian_sgd_polish_max_steps`, and `hessian_sgd_polish_max_ls`. Steps are projected to rate bounds and skip Armijo loss probes before polish and Adam warmup. |
