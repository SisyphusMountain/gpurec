...
The workflow default is `clade_budget=315000`, the retained resident-batch
budget used by the current HOGENOM and `test_trees_1000` production evidence.
...
| `adagrad-restarts-lbfgsb` | Specieswise multifidelity Adagrad prefix, then L-BFGS-B tail. | Requires `mode=specieswise`; uses `adagrad_restart_schedule` and optional `adagrad_restart_phase_loss_patience` for the Adagrad prefix, then continues with `lbfgs_lr`, `loss_change_tol`, `loss_patience`, `projected_grad_tol`, `loss_stop_projected_grad_gate`, optional scheduled loss-change tolerance schedule via `--lbfgsb-loss-change-tol-schedule`, `--lbfgsb-loss-schedule-force-fallback`, and optional late-tail high-KKT/fallback guards, including a fallback loss-evaluation budget, fallback-resolution-competition tuning, schedule-transition fallback forcing, and best-checkpoint retry attempts. The tail resets solver iterations to the run's `fixed_iters_e`, `fixed_iters_pi`, and `neumann_terms`; final validation still uses `adagrad_restart_final_check_iters`. |
...
`docs/release-notes.md`, `docs/support-policy.md`, and top-level `LICENSE`.
