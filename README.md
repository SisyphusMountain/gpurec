...
| `hessian-sgd` | Projected Hessian-conditioned gradient steps for genewise batches. | Requires `mode=genewise`; finite-difference 3x3 row Hessians refresh every `fd_hessian_refresh_steps`, remain fixed between refreshes, and precondition fresh gradients with step scale `lr`. Steps use the same projection, damping, and Armijo acceptance checks as `adam-fd-newton`, but do not apply Adam warmup or BFGS Hessian updates. |
