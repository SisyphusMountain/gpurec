...
`neumann_term_tol` is written in units of float32 precision and is rescaled by
the ratio of machine epsilons for the dtype a solve actually runs in, so a
float64 run exits only below one float64 ulp and float32 behaviour is
bit-identical.
