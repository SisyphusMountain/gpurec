...
def compute_DTS_independent(Pi, Pibar, Pi_D, split_leftrights_sorted, N_splits, S, Pi_s12, Pi_S_s1, Pi_S_s2, log_split_probs):
    # version of compute_DTS when we have
    # different D, T, S, L parameters for each gene tree (independent reconciliations)
    #import IPython; IPython.embed(); exit(1)

    # No difference for these 4
    split_lefts_sorted, split_rights_sorted = torch.chunk(split_leftrights_sorted, 2, dim=0)
    Pi_leftright = torch.index_select(Pi, 0, split_leftrights_sorted) # [2 N_splits, S]
    Pi_left, Pi_right = torch.chunk(Pi_leftright, 2, dim=0)  # Each [N_splits, S]
    ...
    Pi_D_left = torch.index_select(Pi_D, 0, split_lefts_sorted) # [2 N_splits, S]

    DTS = torch.empty((5, N_splits, S), device=Pi.device, dtype=Pi.dtype)
   
    # Duplication term . One side is multiplied by pD, not the other.
    DTS[0] = Pi_D_left + Pi_right
    # Transfer terms. The multiplication by pT is contained in Pibar
    DTS[1] = Pi_left + Pibar_right
    DTS[2] = Pi_right + Pibar_left

    # Speciation terms.
    Pi_S_s1_left = torch.index_select(Pi_S_s1, 0, split_lefts_sorted) # [2 N_splits, 2 S]
    Pi_S_s2_left = torch.index_select(Pi_S_s2, 0, split_lefts_sorted) # [2 N_splits, 2 S]

    Pi_s12_right = torch.index_select(Pi_s12, 0, split_rights_sorted) # [N_splits, 2 S]
    Pi_s1_right = Pi_s12_right[:, :S]
    Pi_s2_right = Pi_s12_right[:, S:]

    # The log_pS multiplication is contained in Pi_S_s1_left and Pi_S_s2_left
    DTS[3] = Pi_S_s1_left + Pi_s2_right
...
def compute_DTS_L_independent(rep_log_pS, Pi, Pibar, E, E_s1, E_s2, Pi_S_s1, Pi_S_s2, Ebar, Pi_D, clade_species_map, log_2, clades_per_gene):
    ...
    DTS_L = torch.empty((6, Pi.shape[0], Pi.shape[1]), dtype=Pi.dtype, device=Pi.device)

    # Repeat per-gene species vectors to clade level [C,S]
    C = Pi.shape[0]
    E_rep = torch.repeat_interleave(E, clades_per_gene, dim=0, output_size=C)
    Ebar_rep = torch.repeat_interleave(Ebar, clades_per_gene, dim=0, output_size=C)
    E_s1_rep = torch.repeat_interleave(E_s1, clades_per_gene, dim=0, output_size=C)
    E_s2_rep = torch.repeat_interleave(E_s2, clades_per_gene, dim=0, output_size=C)

    # DL
    DTS_L[0] = log_2 + E_rep + Pi_D
    # TL
    DTS_L[1] = Pi + Ebar_rep
    DTS_L[2] = Pibar + E_rep
    # SL
    DTS_L[3] = Pi_S_s1 + E_s2_rep
