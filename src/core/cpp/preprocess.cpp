...
py::dict preprocess(const std::string &species_path,
  ...
  leaf_row_index.reserve(C);
  leaf_col_index.reserve(C);
...
// New API: separate species preprocessing and per-gene preprocessing
PYBIND11_MODULE(preprocess_species_gene_cpp, n) {
  n.def("preprocess_species", [](const std::string &species_path) {
    ...
    std::vector<int64_t> s_P_indexes_ext = s_P_indexes;
    for (int64_t idx : s_P_indexes) s_P_indexes_ext.push_back(idx + species_data.S);
    std::vector<int64_t> s_C12_indexes = s_C1_indexes;
    s_C12_indexes.insert(s_C12_indexes.end(), s_C2_indexes.begin(), s_C2_indexes.end());

    auto to_double_matrix = [&](const std::vector<double> &mat, int rows, int cols) {
      return torch::from_blob(const_cast<double *>(mat.data()), {rows, cols}, torch::TensorOptions().dtype(torch::kFloat64)).clone();
    };
    auto to_long_tensor = [&](const std::vector<int64_t> &vec) {
      return torch::from_blob(const_cast<int64_t *>(vec.data()), {static_cast<long>(vec.size())}, torch::TensorOptions().dtype(torch::kInt64)).clone();
    };
    ...
    species_dict["species_name_to_index"] = species_name_to_index;
    return species_dict;
  }, "Preprocess species tree only, return species helpers");

  n.def("preprocess_gene_with_species", [](py::dict species_dict, const std::string &gene_path) {
    // Extract species helpers
    int S = species_dict["S"].cast<int>();
    auto name_map = species_dict["species_name_to_index"].cast<std::unordered_map<std::string,int>>();
    // Parse gene tree and build clades/splits
    auto gene_root = parse_newick_file(gene_path);
    std::vector<std::string> leaf_names;
    std::unordered_map<std::string, int> leaf_to_index;
    collect_leaf_names(gene_root.get(), leaf_names, leaf_to_index);
    const int num_leaves = static_cast<int>(leaf_names.size());
    if (num_leaves == 0) throw std::runtime_error("Gene tree has no leaves");
    const size_t num_words = (static_cast<size_t>(num_leaves) + 63ULL) >> 6;
    ...
    std::unordered_map<TreeNode *, size_t> node_index;
    node_index.reserve(num_nodes);
    for (size_t i = 0; i < num_nodes; ++i) node_index[postorder_nodes[i]] = i;
    std::unordered_map<BitVec, int, BitVecHash, BitVecEqual> clade_to_id;
    std::vector<BitVec> id_to_clade;
    std::vector<int> clade_sizes;
    auto get_or_add = [&](const BitVec &bits) {
      auto it = clade_to_id.find(bits);
      if (it != clade_to_id.end()) return it->second;
      BitVec stored = bits;
      int size = bit_count(stored);
      int new_id = static_cast<int>(id_to_clade.size());
      id_to_clade.push_back(std::move(stored));
    ...
    for (TreeNode *node : postorder_nodes) {
      ...
      if (node->children.empty()) {
        ...
        const BitVec &left = node_clades[node_index[node->children[0]]];
        const BitVec &right = node_clades[node_index[node->children[1]]];
        for (size_t w = 0; w < num_words; ++w) bits[w] = left[w] | right[w];
      }
      node_clades[idx] = bits;
      node_clade_ids[idx] = get_or_add(node_clades[idx]);
    }
    ...
    // Enumerate splits: root and internals
    std::vector<SplitRecord> splits;
    splits.reserve(num_nodes * 3);
    std::unordered_set<PairKey, PairKeyHash, PairKeyEqual> root_split_keys;
    root_split_keys.reserve(num_nodes * 2);
    ...
    auto canonical_key = [&](int a_id, const BitVec &a_bits, int b_id, const BitVec &b_bits) {
      size_t a_idx = static_cast<size_t>(a_id);
      size_t b_idx = static_cast<size_t>(b_id);
      if (clade_sizes[a_idx] < clade_sizes[b_idx]) return PairKey{a_id, b_id};
      if (clade_sizes[a_idx] > clade_sizes[b_idx]) return PairKey{b_id, a_id};
      if (bitvec_lex_less(a_bits, b_bits)) return PairKey{a_id, b_id};
      if (bitvec_lex_less(b_bits, a_bits)) return PairKey{b_id, a_id};
      int first = std::min(a_id, b_id);
      int second = std::max(a_id, b_id);
      return PairKey{first, second};
    };
    for (TreeNode *node : postorder_nodes) {
      if (node == gene_root.get()) continue;
      size_t idx = node_index[node];
      const BitVec &below_bits = node_clades[idx];
      BitVec above_bits = bit_difference(root_bits, below_bits);
      if (is_empty(above_bits)) continue;
      int below_id = node_clade_ids[idx];
      int above_id = ensure_clade(above_bits);
      const BitVec &above_clade = id_to_clade[static_cast<size_t>(above_id)];
      PairKey key = canonical_key(below_id, below_bits, above_id, above_clade);
      if (root_split_keys.insert(key).second) splits.push_back({root_clade_id, below_id, above_id, 1.0});
    }
    for (TreeNode *node : postorder_nodes) {
      if (node == gene_root.get()) continue;
    ...
    // Assemble CCP tensors
    const size_t C = id_to_clade.size();
    const size_t N_splits = splits.size();
    std::vector<int64_t> split_parents(N_splits), split_lefts(N_splits), split_rights(N_splits);
    std::vector<double> split_weights(N_splits), split_probs(N_splits), log_split_probs(N_splits);
    std::vector<int64_t> split_counts(C, 0), parents_sorted(C), split_order(N_splits), parent_rank(C);
    for (size_t i = 0; i < N_splits; ++i) {
      split_parents[i] = splits[i].parent;
      split_lefts[i] = splits[i].left;
      split_rights[i] = splits[i].right;
    ...
    std::vector<double> sum_weights(C, 0.0);
    for (size_t i = 0; i < N_splits; ++i) sum_weights[split_parents[i]] += split_weights[i];
    for (size_t i = 0; i < N_splits; ++i) {
      double denom = sum_weights[split_parents[i]];
      split_probs[i] = denom > 0.0 ? split_weights[i] / denom : 0.0;
      log_split_probs[i] = split_probs[i] > 0.0 ? std::log(split_probs[i]) : -std::numeric_limits<double>::infinity();
    }
    std::iota(split_order.begin(), split_order.end(), 0);
    std::iota(parents_sorted.begin(), parents_sorted.end(), 0);
    std::stable_sort(parents_sorted.begin(), parents_sorted.end(), [&](int64_t a, int64_t b){ if (split_counts[a] != split_counts[b]) return split_counts[a] > split_counts[b]; return a < b; });
    for (size_t i = 0; i < C; ++i) parent_rank[parents_sorted[i]] = static_cast<int64_t>(i);
    std::stable_sort(split_order.begin(), split_order.end(), [&](int64_t lhs, int64_t rhs){ int64_t ra = parent_rank[split_parents[lhs]]; int64_t rb = parent_rank[split_parents[rhs]]; if (ra != rb) return ra < rb; return lhs < rhs; });
    std::vector<int64_t> split_parents_sorted(N_splits), split_lefts_sorted(N_splits), split_rights_sorted(N_splits);
    std::vector<double> log_split_probs_sorted(N_splits);
    for (size_t i = 0; i < N_splits; ++i) {
      size_t idx = static_cast<size_t>(split_order[i]);
      split_parents_sorted[i] = split_parents[idx];
      split_lefts_sorted[i] = split_lefts[idx];
      split_rights_sorted[i] = split_rights[idx];
      log_split_probs_sorted[i] = log_split_probs[idx];
    }
    std::vector<int64_t> split_leftrights_sorted(N_splits * 2);
    for (size_t i = 0; i < N_splits; ++i) { split_leftrights_sorted[i] = split_lefts_sorted[i]; split_leftrights_sorted[i + N_splits] = split_rights_sorted[i]; }
    std::vector<int64_t> seg_counts(C);
    std::vector<int64_t> ptr(C + 1);
    for (size_t i = 0; i < C; ++i) seg_counts[i] = split_counts[parents_sorted[i]];
    ptr[0] = 0; for (size_t i = 0; i < C; ++i) ptr[i + 1] = ptr[i] + seg_counts[i];
    int num_segs_ge2 = 0, num_segs_eq1 = 0, num_segs_eq0 = 0;
    for (int64_t c : seg_counts) { if (c >= 2) ++num_segs_ge2; else if (c == 1) ++num_segs_eq1; else ++num_segs_eq0; }
    int stop_reduce_ptr_idx = num_segs_ge2;
    int end_rows_ge2 = static_cast<int>(ptr[stop_reduce_ptr_idx]);
    std::vector<int64_t> ptr_ge2(ptr.begin(), ptr.begin() + stop_reduce_ptr_idx + 1);

    auto to_long_tensor = [&](const std::vector<int64_t> &vec) { return torch::from_blob(const_cast<int64_t *>(vec.data()), {static_cast<long>(vec.size())}, torch::TensorOptions().dtype(torch::kInt64)).clone(); };
    auto to_double_tensor = [&](const std::vector<double> &vec) { return torch::from_blob(const_cast<double *>(vec.data()), {static_cast<long>(vec.size())}, torch::TensorOptions().dtype(torch::kFloat64)).clone(); };

    // Build clade_species_map
    // torch::Tensor clade_species_tensor = torch::zeros({static_cast<long>(C), S}, torch::TensorOptions().dtype(torch::kFloat64));
    // auto tensor_accessor = clade_species_tensor.accessor<double, 2>();
    std::vector<int64_t> leaf_row_index;
    std::vector<int64_t> leaf_col_index;
    leaf_row_index.reserve(C);
    ...
    for (size_t cid = 0; cid < C; ++cid) {
      ...
      for (size_t w = 0; w < bits.size(); ++w) {
        ...
        while (word) {
          unsigned long b = __builtin_ctzll(word);
          size_t leaf_idx = w * 64 + b;
          if (leaf_idx < leaf_names.size()) {
            indices.push_back(static_cast<int64_t>(leaf_idx));
            if (clade_sizes[cid] == 1) {
              const std::string &leaf_name = leaf_names[leaf_idx];
              std::string species = extract_species_name(leaf_name);
              auto it = name_map.find(species);
              if (it == name_map.end()) throw std::runtime_error("Species " + species + " not found for gene leaf " + leaf_name);
              //tensor_accessor[static_cast<long>(cid)][it->second] = 1.0;
              leaf_row_index.push_back(static_cast<int64_t>(cid));
              leaf_col_index.push_back(static_cast<int64_t>(it->second));
      ...
      std::sort(indices.begin(), indices.end());
      if (clade_sizes[cid] == 1) clade_is_leaf[cid] = 1;
    }

    py::dict ccp_dict;
    ccp_dict["split_counts"] = to_long_tensor(split_counts);
    ccp_dict["split_order"] = to_long_tensor(split_order);
    ccp_dict["split_parents_sorted"] = to_long_tensor(split_parents_sorted);
    ccp_dict["split_lefts_sorted"] = to_long_tensor(split_lefts_sorted);
    ccp_dict["split_rights_sorted"] = to_long_tensor(split_rights_sorted);
    ccp_dict["split_leftrights_sorted"] = to_long_tensor(split_leftrights_sorted);
    ccp_dict["log_split_probs_sorted"] = to_double_tensor(log_split_probs_sorted);
    ccp_dict["parents_sorted"] = to_long_tensor(parents_sorted);
    ccp_dict["seg_parent_ids"] = to_long_tensor(parents_sorted);
    ccp_dict["seg_counts"] = to_long_tensor(seg_counts);
    ccp_dict["ptr"] = to_long_tensor(ptr);
    ccp_dict["ptr_ge2"] = to_long_tensor(ptr_ge2);
    ccp_dict["num_segs_ge2"] = num_segs_ge2;
    ccp_dict["num_segs_eq1"] = num_segs_eq1;
    ccp_dict["num_segs_eq0"] = num_segs_eq0;
    ccp_dict["stop_reduce_ptr_idx"] = stop_reduce_ptr_idx;
    ...
    ccp_dict["C"] = static_cast<int64_t>(C);
    ccp_dict["N_splits"] = static_cast<int64_t>(N_splits);
    // clade leaves and labels
    ccp_dict["clade_leaves"] = clade_leaves_indices;
    ccp_dict["clade_leaf_labels"] = clade_leaf_labels;
    ccp_dict["clade_is_leaf"] = torch::from_blob(clade_is_leaf.data(), {static_cast<long>(C)}, torch::TensorOptions().dtype(torch::kUInt8)).clone();

    py::dict out;
    out["ccp"] = ccp_dict;

    // out["clade_species_map"] = clade_species_tensor;
    out["root_clade_id"] = root_clade_id;
    out["leaf_row_index"] = to_long_tensor(leaf_row_index);
    out["leaf_col_index"] = to_long_tensor(leaf_col_index);
    return out;
  }, "Build CCP and clade_species_map for one gene tree using precomputed species helpers");
}
