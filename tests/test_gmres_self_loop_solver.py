...
def test_gmres_large_direct_sum_max_tiles_env(monkeypatch):
    ...
    assert wave_backward._gmres_can_use_triton_large_direct_sum(4096)
    assert not wave_backward._gmres_can_use_triton_large_direct_sum(4097)

    monkeypatch.setenv("GPUREC_GMRES_TRITON_LARGE_DIRECT_SUM_MAX_TILES", "2048")
    assert wave_backward._gmres_can_use_triton_large_direct_sum(2048)
...
def test_gmres_self_loop_triton_split_can_trust_min_check_iter_cuda_float32(monkeypatch):
    ...
    assert stats[0]["trusted_finalize_write"] is True
...
def test_gmres_self_loop_triton_large_can_trust_min_check_iter_cuda_float32(monkeypatch):
    ...
    assert stats[0]["trusted_check_schedule"] is True
    assert stats[0]["trusted_check_used"] is True
    assert stats[0]["trusted_finalize_write"] is False


def test_gmres_self_loop_triton_large_trusted_finalize_write_group_norm_cuda_float32(monkeypatch):
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for the Triton GMRES solve path")
    ...
    assert stats and stats[0]["arnoldi_backend"] == "triton_large"
    assert stats[0]["large_direct_sum"] is False
    assert stats[0]["large_group_norm_reductions"] == 2
    assert stats[0]["iterations"] == 2
    assert stats[0]["trusted_check_schedule"] is True
    assert stats[0]["trusted_check_used"] is True
    assert stats[0]["trusted_finalize_write"] is True
