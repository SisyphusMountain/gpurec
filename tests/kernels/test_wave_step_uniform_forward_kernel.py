...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_wave_step_uniform_two_kernel_leaf_index_matches_dense_leaf_term_specieswise():
    ...
    Pibar_dense = torch.full_like(Pi, float("-inf"))
    Pi_dense, max_diff_dense = wave_step_uniform_two_kernel_fused(
        Pi,
        Pibar_dense,
        ws,
        W,
    ...
    Pibar_indexed = torch.full_like(Pi, float("-inf"))
    Pi_indexed, max_diff_indexed = wave_step_uniform_two_kernel_fused(
        Pi,
        Pibar_indexed,
        ws,
        W,
