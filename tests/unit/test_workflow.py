...
def test_effective_route_metadata_reports_production_likelihood_contract(
    ...
    assert effective_final_check_iters_e(config) is None
    assert route["final_check_iters_e"] is None
...
def test_run_config_auto_optimizer_uses_adagrad_restarts_for_specieswise_mode(
    ...
    assert effective_final_check_iters_e(config) == 128
    route = effective_route_metadata(config)
    ...
    assert route["final_check_iters"] == effective_final_check_iters(config)
    assert route["final_check_iters_e"] == effective_final_check_iters_e(config)
    assert route["uses_production_default_optimizer_settings"] is True
    assert route["production_default_optimizer_setting_mismatches"] == []
    assert route["uses_production_default_route"] is True
    assert route["production_default_route_mismatches"] == []
...
def test_effective_final_check_iters_uses_optimizer_specific_budget(
    ...
    specieswise_lbfgsb = RunConfig(
        species_tree=tmp_path / "sp.nwk",
        families_file=tmp_path / "families.txt",
        out_dir=tmp_path / "specieswise-lbfgsb-out",
        mode="specieswise",
        optimizer="lbfgsb",
        device="cpu",
        fixed_iters_e=6,
    ...
    assert effective_final_check_iters_e(genewise) is None
    assert effective_route_metadata(genewise)["final_check_iters_e"] is None
    assert effective_final_check_iters(specieswise) == 32
    assert effective_route_metadata(specieswise)["final_check_iters"] == 32
    assert effective_final_check_iters_e(specieswise) == 32
    assert effective_route_metadata(specieswise)["final_check_iters_e"] == 32
    assert effective_final_check_iters(specieswise_lbfgsb) == 32
    assert effective_final_check_iters_e(specieswise_lbfgsb) == 32
    assert effective_route_metadata(specieswise_lbfgsb)["final_check_iters_e"] == 32
    ...
    assert effective_final_check_iters_e(disabled_specieswise) == 0
    assert effective_route_metadata(disabled_specieswise)["final_check_iters_e"] == 0
        ...
        solver_stat_records=lambda: [
            {
                ...
                "Pi_adjoint_residual_absmax": 0.5,
                "Pi_adjoint_residual_relmax": 0.25,
                "Pi_adjoint_residual_wave_count": 4,
            },
            {
                "E_iterations": 3,
                "Pi_max_iterations": 6,
                ...
                "Pi_adjoint_residual_relmax": 0.05,
...
def test_optimization_runner_lbfgsb_high_kkt_waits_for_final_loss_phase(
    tmp_path: Path,
):
    class ScriptedLBFGSB:
        def __init__(self, param: torch.nn.Parameter):
    ...
    assert len(optimizer_rows) == 4
    assert optimizer_rows[1]["optimizer/lbfgsb_high_kkt_final_loss_phase"] is False
    assert optimizer_rows[1]["optimizer/lbfgsb_high_kkt_stop_ready"] is False
    assert optimizer_rows[2]["optimizer/lbfgsb_loss_schedule_advance"] is True
    assert optimizer_rows[3]["optimizer/lbfgsb_high_kkt_final_loss_phase"] is True
    assert optimizer_rows[3]["optimizer/lbfgsb_high_kkt_stop_ready"] is True
    assert result.status == "converged"
...
def test_optimization_runner_lbfgsb_can_stop_before_second_high_kkt_fallback(
    ...
    class ScriptedLBFGSB:
        ...
        def step(self, closure, *, loss_closure=None):
            ...
            param = self.param_groups[0]["params"][0]
            flat_grad = torch.ones_like(param.detach()).reshape(-1)
            high_kkt_count = 0 if self.calls == 1 else 2
            fallback_used = self.calls == 1
            self.state[param].update(
                {
                    "last_loss": torch.as_tensor(
                        10.0 if self.calls == 1 else 9.875,
                        dtype=param.dtype,
                        device=param.device,
                    ),
                    "last_grad": flat_grad,
                    ...
                    "last_step_inf": 0.0,
                    "last_direction_kind": "subspace",
                    "last_line_search_decrease": 0.125,
                    "last_armijo_required_decrease": 0.0,
                    ...
                    "last_high_kkt_stall_count": high_kkt_count,
                    "last_history_cleared_for_fallback": fallback_used,
                }
            )
            return self.state[param]["last_loss"]
...
def test_optimization_runner_lbfgsb_can_stop_on_budget_exhausted_high_kkt_plateau(
    tmp_path: Path,
):
    class ScriptedLBFGSB:
        def __init__(self, param: torch.nn.Parameter):
            ...
            self.state[param].update(
                {
                    ...
                    "last_fallback_max_loss_evals": 4,
                    "last_fallback_budget_exhausted": True,
                    "last_fallback_reason": "line_search_failed",
                    "last_high_kkt_stall_count": 1,
                    "last_history_cleared_for_fallback": True,
                }
            )
            return self.state[param]["last_loss"]
...
def test_optimization_runner_rejects_nonfinite_active_adam_warmup_update(
    tmp_path: Path,
    monkeypatch,
):
    def make_theta_nonfinite(self, closure=None):
