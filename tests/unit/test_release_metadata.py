        ...
        (root / "docs" / "release-readiness.md").write_text(
            "\n".join(
                [
                    ...
                    "Semantic versioning expectations are documented before publication.",
        ...
        (root / "docs" / "support-policy.md").write_text(
            "\n".join(
                [
                    ...
                    "Support scope follows api-contract.md guarantees.",
                    "Support window references platform-matrix.md as the environment source of truth.",
                    ...
                    "Support evidence includes installed version/tag details.",
                    "Support evidence includes installation method details.",
        ...
        (root / "docs" / "validation-envelope.md").write_text(
            "\n".join(
                [
                    "# Validation Envelope",
                    "",
                    "Runtime envelope, peak memory evidence, final NLL range, benchmark evidence scope,",
                    "convergence status tracking, and sampling output shape checks.",
                    "",
                ]
            ),
            encoding="utf-8",
        ...
        (root / "docs" / "troubleshooting.md").write_text(
            "\n".join(
                [
                    ...
                    "Resume with unchanged config when checkpoint and route metadata match.",
                    "",
                ]
            ),
            encoding="utf-8",
...
def test_release_metadata_check_requires_support_policy_support_window_phrases(
    ...
    assert "must document support-window phrase: platform-matrix.md" in result.stdout
    assert "must document support-window phrase: run_config.json" in result.stdout
    assert "must document support-window phrase: history.jsonl" in result.stdout
    ...
    assert "must document support-window phrase: version/tag" in result.stdout
...
def test_release_metadata_check_requires_platform_matrix_core_terms(
    ...
    assert "must document matrix term: source-only" in result.stdout
...
def test_release_metadata_check_requires_validation_envelope_evidence_terms(
    ...
    assert "must document validation evidence term: convergence status" in result.stdout
    assert "must document validation evidence term: sampling output shape" in result.stdout
    assert result.stderr == ""
...
def test_release_metadata_check_requires_troubleshooting_recovery_phrases(
    ...
    assert "must document failure-recovery phrase: unchanged config" in result.stdout
    assert result.stderr == ""
...
def test_release_metadata_check_requires_release_readiness_gate_phrases(
    ...
    assert "must document release gate phrase: semantic versioning expectations" in result.stdout
    ...
    assert "must document release gate phrase: dependency vulnerability scanning" in result.stdout
    assert "must document release gate phrase: fresh machine can reproduce the documented validation run" in result.stdout
    ...
    assert "must document release gate phrase: actionable" in result.stdout
    ...
    assert "must document release gate phrase: exact tested platform matrix" in result.stdout
    assert "must document release gate phrase: benchmark evidence" in result.stdout
    assert "must document release gate phrase: native preprocessing and backtracking are provided" in result.stdout
